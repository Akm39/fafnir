...
data.py - used to call & load data, precompute cov matrixes, betas, and returns (dumped in the data folder)
...
TO DO:
1. all_coefs.py- wrapper for grouptester.py, iterates over all industry groups applying iter.py & valid.py, & compiles coefficient matricies
2. pnl.py - calculates pnl based on old weights -> new weights (transaction costs) & new weights -> next period weights (price change)
3. optimization.py - takes all precomputed data (coef matrix, signals, half lifes, cov matrix, betas, returns) and does a day by day optimization of the portfolio w/weight matrixes computed by coefs.py and pnl computed by pnl.py